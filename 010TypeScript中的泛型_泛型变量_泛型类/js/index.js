...
console.log(getData4(123)); // 指定泛型为数值型：传入参数和返回值都为number类型
console.log(getData4(true)); // 指定泛型为布尔型：传入参数和返回值都为boolean类型
...
var MinClass = /** @class */ (function () {
    ...
    MinClass.prototype.findMin = function () {
        var min = this.list[0];
...
var CMin = /** @class */ (function () {
    function CMin() {
        this.list = [];
    }
    CMin.prototype.add = function (value) {
        this.list.push(value);
    };
    CMin.prototype.findMin = function () {
        var min = this.list[0];
        for (var i = 0; i < this.list.length; i++) {
            this.list[i] < min ? (min = this.list[i]) : (min = min);
        }
...
var charArr = new CMin(); // 实例化一个实例并指定泛型类型
charArr.add("A");
charArr.add("Z");
charArr.add("T");
charArr.add("D");
